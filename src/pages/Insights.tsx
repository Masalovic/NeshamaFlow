...
export default function Insights() {
  ...
  const last28 = useMemo(() => {
    const list = history ?? []
    const cutoff = dayjs().subtract(27, 'day').startOf('day')
    return list.filter(x => dayjs(x.ts).isAfter(cutoff))
  }, [history])
  ...
  const blocks: Block[] = useMemo(() => {
    ...
    // show only non-empty blocks, sorted by count desc
     ...
     <div className="flex min-h-full flex-col">
      ...
      <main className="flex-1 p-4">
        <div className="max-w-[480px] mx-auto space-y-4 pb-24">
          ...
          {/* Mood distribution */}
          <Card title="Last 28 days — mood distribution" right={!!moodChart.length && <span className="text-xs text-gray-500">{last28.length} entries</span>}>
            {moodChart.length ? (
              <div className="h-56">
                  ...
                  <tbody>
                    {quickWins.map((r) => (
                      <tr key={r.ritualId} className="border-t">
                        <td className="py-2 pr-3 font-medium">{r.ritualId}</td>
